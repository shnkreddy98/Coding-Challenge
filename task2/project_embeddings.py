...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Project bilinear embeddings using a trained linear probe"
    )
    parser.add_argument(
        "-p", "--projection", required=True, help="Path to projection.pt"
    )
    ...
    for dataset, crop, crop_path in all_crops:
        ...
        with torch.no_grad():
            for z_idx in range(Z):
                flat = torch.tensor(
                    embeddings[z_idx].astype(np.float32).reshape(H * W, D)
                )
