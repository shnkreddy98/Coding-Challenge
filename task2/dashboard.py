...
if not crops:
    st.warning(
        f"No projected embeddings found in {query_dataset}. Run project_embeddings.py first."
    )
    st.stop()
...
intra_data = (
    cached_load_crop(intra_dataset, intra_crop)
...
inter_data = (
    cached_load_crop(inter_dataset, inter_crop)
...
with col2:
    ...
    if intra_data is not None:
        ...
        if st.session_state.query_emb is not None:
            sim_intra = compute_similarity(
                st.session_state.query_emb, intra_data["embeddings"]
            )
...
with col3:
    ...
    if inter_data is not None:
        ...
        if st.session_state.query_emb is not None:
            sim_inter = compute_similarity(
                st.session_state.query_emb, inter_data["embeddings"]
            )
